...
class StorageManage extends PureComponent {
  ...
  handleSubmitStorageConfig = (vals, data) => {
    ...
    const { attr_default_value } = newData;
    ...
    newData.attr_default_value = str && JSON.stringify(str);
  ...
  render() {
    ...
    storageList.length > 0 &&
      storageList.map(item => {
        const { attr_default_value } = item;
        const str =
          (attr_default_value && JSON.parse(attr_default_value)) || {};
...
class PluginConfigs extends PureComponent {
  ...
  render() {
    console.log(this.props, 'props1234');
